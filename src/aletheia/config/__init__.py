...
def data_directory(*, ignore_environment: bool = False) -> Path:
    override = os.getenv("ALETHEIA_DATA_DIR")
    if override and not ignore_environment:
        return Path(override).expanduser()
    if sys.platform == "darwin":
        return Path.home() / "Library/Application Support/Aletheia"
    if sys.platform == "win32":
        return Path(os.getenv("APPDATA", str(Path.home()))) / "Aletheia"
    return Path(os.getenv("XDG_DATA_HOME", str(Path.home() / ".local/share"))) / "Aletheia"
...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(
        env_prefix="ALETHEIA_", env_nested_delimiter="__", extra="forbid"
    )
    ...
    def load(cls, path: Path | None = None) -> Settings:
        ...
        for env_key, env_value in os.environ.items():
            if not env_key.startswith("ALETHEIA_"):
                continue
            keys = env_key.removeprefix("ALETHEIA_").lower().split("__")
