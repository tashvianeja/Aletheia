...
def install(settings: Settings, configure_autostart: bool = True) -> list[Path]:
    ...
    if sys.platform == "win32":
        executable = (
            Path(sys.executable).with_name("AletheiaHost.exe")
            if getattr(sys, "frozen", False)
            else Path(sys.executable).parent / "aletheia-host.exe"
        )
    else:
        executable = settings.data_dir / "aletheia-host"
        command = (
            [sys.executable, "--native-host"]
            if getattr(sys, "frozen", False)
            else [sys.executable, "-m", "aletheia.core.ipc.native_host"]
        )
    ...
    for browser in ("chrome", "edge", "brave", "firefox"):
        manifest: dict[str, object] = {
            "name": HOST_NAME,
            "description": "Local Aletheia analysis bridge",
            "path": str(executable),
            "type": "stdio",
        }
        if browser == "firefox":
    ...
    if configure_autostart and settings.autostart:
        from aletheia.sensors.platform import create_adapter
...
def uninstall(settings: Settings, remove_data: bool = True) -> None:
    if sys.platform == "darwin":
        ...
        subprocess.run(
            ["/bin/launchctl", "bootout", f"gui/{os.getuid()}/com.aletheia.app"],
        ...
        (Path.home() / "Library/LaunchAgents/com.aletheia.app.plist").unlink(missing_ok=True)
        ...
        from aletheia.sensors.platform.windows import WindowsRegistry
    ...
    if remove_data and settings.data_dir.is_dir() and settings.data_dir.name != "":
        ...
        for folder_name in ("chrome", "edge", "brave", "firefox", "logs"):
            ...
            if target.is_dir() and not target.is_symlink():
                ...
                with contextlib.suppress(OSError):
                    target.rmdir()
        from aletheia.core.ipc.transport import endpoint
        ...
        if settings.data_dir.resolve() == data_directory(ignore_environment=True).resolve():
            with contextlib.suppress(Exception):
                from aletheia.llm.client import delete_api_key
