...
from aletheia.analysis.documents import (
    ExtractedDocument,
    RedactionResult,
    extract_document,
    redact_document,
...
from aletheia.analysis.documents.extract import MAX_BYTES, SAMPLE_BYTES
from aletheia.analysis.forms import FieldAssessment, analyze_fields
from aletheia.analysis.pii import detect_pii
from aletheia.analysis.policy import analyze_policy, analyze_terms
from aletheia.analysis.tracking import TrackingSnapshot, analyze_tracking
from aletheia.core.events import DataCategory, Finding, FormContext, FormField
...
def warm_analysis() -> dict[str, bool]:
    from aletheia.analysis.pii.detector import _ner
    from aletheia.analysis.policy.analyzer import _segmenter
...
def count_redaction_marks(data: bytes, filename: str) -> int:
    """How many Aletheia boxes a file on disk carries; 0 when none or unreadable."""
