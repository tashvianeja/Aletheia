...
from aletheia.analysis.pii import redact_text
from aletheia.core.events import DataCategory
from aletheia.engine.labels import article, category_label, lowered, purpose_label
...
def patterns() -> dict[str, dict[str, list[str]]]:
    value: dict[str, dict[str, list[str]]] = json.loads(
        files("aletheia.analysis.policy").joinpath("patterns/clauses.yaml").read_text()
    )
    return value
