...
def findings_for(
    ...
    from aletheia.engine.explain import consent_findings, form_findings
    ...
    if isinstance(event, ClipboardReadEvent):
        from aletheia.engine.labels import article, category_label, lowered
...
def policy_rows(clauses: list[object], nothing_unusual: list[str]) -> list[DecisionFinding]:
    ...
    """
    from aletheia.engine.clauses import clause_meaning, clause_title, is_material
