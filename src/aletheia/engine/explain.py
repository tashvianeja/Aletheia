...
from aletheia.engine.labels import (
    LABELS,
    category_label,
    lowered,
    plural_label,
...
from aletheia.engine.labels import article as _article
...
def form_task(event: FormObservedEvent) -> str:
    ...
    from aletheia.intelligence.intent import classify_form
    from aletheia.intelligence.necessity import INTENT_PHRASES, MIN_INTENT_CONFIDENCE
...
def _headline(
    ...
    if isinstance(event, ConsentBannerEvent):
        ...
        # "Aletheia can reject the optional cookies for you" is what the
    ...
    if isinstance(event, RedactedDocumentEvent):
        ...
        headline = f"{event.filename or 'This file'} has {boxes} drawn by Aletheia."
        body = (
...
def explain(
    ...
    from aletheia.engine.presentation import tracking_mechanisms
