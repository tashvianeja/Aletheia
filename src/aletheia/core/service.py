...
from aletheia.util.privacy import public_identity, safe_origin, sanitize
...
class Service:
    ...
    def prepare_browser_worker(self) -> None:
        ...
        async def prepare() -> None:
            from aletheia.core.worker_dispatch import prepare_worker
    ...
    def prepare_upload_analysis(self) -> None:
        ...
        async def prepare() -> None:
            from aletheia.core.worker_dispatch import prepare_upload_model
        ...
        while True:
            ...
            try:
                ...
                if self._embedder_loaded:
                    from aletheia.intelligence.embedder import release_if_idle
    ...
    async def process_event(
        ...
        from aletheia.analysis.purpose import enrich_requester
        from aletheia.core.events import FormObservedEvent, FormSubmitEvent
        ...
        if event.event_type == "policy_document":
            ...
            from aletheia.engine.clauses import clause_meaning, clause_title
            from aletheia.engine.clauses import ordinary_label as ordinary
        ...
        from aletheia.engine.necessity import Necessity, necessity_for
    ...
    def _suggest_learned_default(self, response: UserResponse, event: PrivacyEvent) -> None:
        ...
        if not action:
            return
        from aletheia.engine.preferences import AUTOMATABLE
        ...
        suggestion = Decision(
            ...
            rationale=[
                "Aletheia only offers this for choices it can reverse, "
    ...
    async def _refine_decision(self, event: PrivacyEvent, decision: Decision) -> None:
        ...
        from aletheia.core.worker_dispatch import refine_context
    ...
    async def _refine_public(
        ...
        from aletheia.llm.enrichment import refine_public_profile
    ...
    async def _respond_once(self, response: UserResponse) -> dict[str, Any]:
        ...
        if response.action in {"redact", "strip_metadata"}:
            from aletheia.analysis.worker import redact_payload
            ...
            folder = Path.home() / "Downloads/Aletheia"
            folder.mkdir(parents=True, exist_ok=True, mode=0o700)
            from aletheia.util.permissions import secure_path
        ...
        if response.action == "unredact":
            from aletheia.analysis.worker import unredact_payload
        ...
        if event.payload_ref and response.action in {
            ...
            from aletheia.analysis.worker import release_payload
    ...
    async def _route(self, request: Request) -> dict[str, Any]:
        ...
        if request.type == "fetch_document":
            from aletheia.util.public_document import fetch_public_document

            return await asyncio.wait_for(
                fetch_public_document(
                    str(payload["url"]),
                    str(payload["origin"]),
                    str(payload.get("user_agent", "Aletheia")),
                ),
                timeout=3,
            )
        ...
        if request.type == "event":
            from aletheia.sensors.browser_bridge import parse_browser_event
        ...
        if request.type == "file_start":
            ...
            from aletheia.core.worker_dispatch import MAX_SIZE, MAX_UPLOADS
            from aletheia.sensors.browser_bridge import enrich_browser_requester
        ...
        if request.type == "context":
            ...
            for kind in ("forms", "consent", "tracking", "policy", "terms"):
                ...
                from aletheia.analysis.worker import AnalysisResult
                ...
                if cached is not None and isinstance(cached.get(kind), dict):
                    ...
                    if kind == "policy":
                        ...
                        from aletheia.analysis.policy.analyzer import (
        ...
        if request.type == "deep_check":
            from aletheia.deepcheck import run_deep_check
