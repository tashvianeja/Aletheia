...
from aletheia.sensors.platform.base import (
...
class MacOSAdapter(PlatformAdapter):
    ...
    def parse_log(self, record: dict[str, Any]) -> list[PrivacyEvent]:
        ...
        if not service_match or not client_match:
            ...
            if re.search(r"paste|clipboard", message, re.I) and re.search(
                ...
                from aletheia.core.events import ClipboardReadEvent
    ...
    def set_autostart(self, enabled: bool) -> None:
        if sys.platform != "darwin":
            return
        target = Path.home() / "Library/LaunchAgents/com.aletheia.app.plist"
        import os

        domain = f"gui/{os.getuid()}"
        if not enabled:
            subprocess.run(
                ["/bin/launchctl", "bootout", domain + "/com.aletheia.app"],
        ...
        command = (
            ...
            else [sys.executable, "-m", "aletheia"]
        )
        target.write_bytes(
            plistlib.dumps(
                {
                    "Label": "com.aletheia.app",
