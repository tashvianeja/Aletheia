...
class WindowsRegistry:
    ...
    def set_autostart(self, command: str | None) -> None:
        ...
        with winreg.CreateKey(winreg.HKEY_CURRENT_USER, RUN_ROOT) as key:
            if command:
                winreg.SetValueEx(key, "Aletheia", 0, winreg.REG_SZ, command)
            else:
                with contextlib.suppress(FileNotFoundError):
                    winreg.DeleteValue(key, "Aletheia")
...
class WindowsAdapter(PlatformAdapter):
    ...
    def set_autostart(self, enabled: bool) -> None:
        command = f'"{sys.executable}"' + ("" if getattr(sys, "frozen", False) else " -m aletheia")
