"""The Aletheia widget: one card, reused by every kind of intervention.

Layout, in the order the mockups put it:

    ┃ [tier icon] ACT NOW / HEADS UP / ...    Aletheia   [x] ┃  <- coloured band
    Headline in bold
    Body paragraph
...
from aletheia.ui import icons
from aletheia.ui.theme import card_stylesheet, palette, urgency_palette
...
def bring_to_front(widget: QWidget) -> None:
    ...
    Aletheia is an agent app (LSUIElement) with no Dock icon, so macOS never
