...
Cloud assistance is disabled by default (`llm.enabled = false`). The code default model is `gemini-flash-latest`. Preferences offers a short list of suggested models and accepts any identifier typed into the box, because the set of published Gemini models changes faster than this application ships. **Test** in Preferences sends one minimal structured request with the key and model currently on screen; on success it replaces the suggestions with the models that key can actually call (`models.list()`, filtered to those supporting `generateContent`). No model identifier in this repository has been confirmed against a live account: the Test button exists so that you confirm it against yours.
