...
| An extension blocks too much or too little | Bounded dynamic rules, explicit user-facing action, category context, four-second fail-open initial analysis | Browser APIs cannot cover all traffic; all 23 real Chromium cases plus Firefox pass in the final local headed run, but CI remains pending. |
...
IPC tests have reported real Unix-socket coverage; Chromium browser actions and Firefox’s fixed-ID native-host handshake are also exercised. The refreshed macOS artifact lifecycle and the native-host death/result-race regression are exercised. `pip-audit` was clean except for the local project and direct spaCy wheel, which are not PyPI packages and were skipped; Bandit medium-and-above was clean. Real TCC/screen checks, final Windows installer/runtime acceptance, and green CI remain pending. These are material defense-in-depth checks and must be completed before a release claim.
