...
IPC tests have reported real Unix-socket coverage; Chromium browser actions and Firefox’s fixed-ID native-host handshake are also exercised. `pip-audit` was clean except for the local project and direct spaCy wheel, which are not PyPI packages and were skipped; Bandit medium-and-above was clean. Real TCC/screen checks, final Windows installer/runtime acceptance, refreshed artifacts after the native-host race fix, and green CI remain pending. These are material defense-in-depth checks and must be completed before a release claim.
