(() => {
  ...
  document.addEventListener('click',async event=>{
    ...
    try{const result=await PG.safeRace(analyze('terms',true),4000,null);if(!result){if(intendedChecked!==null){target.checked=intendedChecked;target.dispatchEvent(new Event('change',{bubbles:true}));}else{bypass.add(target);target.click();}return;}const decision=result.terms?.decision;const citations=(decision?.findings||[]).map(finding=>finding.quote||'').filter(Boolean);const selected=await PG.awaitDecision(decision,action=>{if(action.action==='view_details')PG.showClauses(citations);});if(selected.action==='continue'||!decision||decision.outcome!=='INTERVENE'){agreed.add(result.document_key);if(!result.partial)await PG.request('document_acknowledge',{key:result.document_key});if(intendedChecked!==null){target.checked=intendedChecked;target.dispatchEvent(new Event('change',{bubbles:true}));}else{bypass.add(target);target.click();}}else if(selected.action==='view_details'){PG.showClauses(citations);}}catch(_){}
  },true);
