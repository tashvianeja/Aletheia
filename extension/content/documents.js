(() => {
  ...
  document.addEventListener('click',async event=>{
    ...
    try{const result=await PG.safeRace(analyze('terms',true),4000,null);if(!result){if(intendedChecked!==null){target.checked=intendedChecked;target.dispatchEvent(new Event('change',{bubbles:true}));}else{bypass.add(target);target.click();}return;}const decision=result.terms?.decision;const selected=await PG.awaitDecision(decision);if(selected.action==='continue'||!decision||decision.outcome!=='INTERVENE'){agreed.add(result.document_key);if(!result.partial)await PG.request('document_acknowledge',{key:result.document_key});if(intendedChecked!==null){target.checked=intendedChecked;target.dispatchEvent(new Event('change',{bubbles:true}));}else{bypass.add(target);target.click();}}}catch(_){}
  },true);
