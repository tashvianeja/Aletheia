(() => {
  ...
  async function snapshot(){await ready;let banner=null,cmp='unknown',selectors=[];
    ...
    if(!banner)banner=PG.queryAll('[role=dialog],[role=region],aside,section,div').slice(0,2000).find(element=>{if(element.closest('.pg-panel,.pg-stack'))return false;const style=getComputedStyle(element);return ['fixed','sticky'].includes(style.position)&&PG.visible(element)&&/cookies?|consent|privacy preferences/i.test(element.textContent||'')&&element.querySelector('button,[role=button]');});
