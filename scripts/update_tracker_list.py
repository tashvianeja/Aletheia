...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Explicitly update the local Tracker Radar domain seed from its GitHub catalogue."
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "src/privacy_guardian/data/trackers.json",
    )
